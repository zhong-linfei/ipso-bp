...
def chaotic_sequence(iterations, initial_value):
    X = np.zeros(iterations)
    X[0] = initial_value

    for i in range(1, iterations):
        X[i] = (X[i-1] + 0.2 - (0.5/(2*np.pi))*np.sin(2*np.pi*X[i-1])) % 1
...
def pso(func, lb, ub, ieqcons=[], f_ieqcons=None, args=(), kwargs={}, 
    ...
    x=np.array(chaotic_sequence(S*D,0.1)).reshape(S,D)
    ...
    while it<=maxiter:
        ...
        omega=omegamax- (it*(omegamax-omegamin) / maxiter  )
        for i in range(S):
